...
def read_motion_data(dir_name, motion_train_list):
    ...
    '''
    # .datファイルのみを取得
    data_files = glob.glob(os.path.join(dir_name, '*.dat'))
    ...
    if len(data_files) > 0:
        ...
        for each_file in data_files:
            ...
            data = np.genfromtxt(os.path.join(each_file), delimiter=",")
            times = int(each_file[-5])  # 各データの動作番号
            motion = {"standing":1, "walking":2, "sidewalking":3}
            label = motion[each_file[0:-8]] # クラスラベル
            if times in motion_train_list:  # 訓練用
                # 入力データ
                train_input = np.vstack((train_input, data[:,0])) # [N_u*4] * n_time
                # 出力データ（n_time x n_label，値はすべて'-1'）
                tmp = np.zeros((n_time, 1, 4))
                ...
                # 入力データ
                test_input = np.vstack((test_input, data[:,0])) # [N_u*4] * n_time
                # 出力データ（n_time x n_label，値はすべて'-1'）
                tmp = np.zeros((n_time, 1, 4))
                tmp[:, :, label] = 1  # labelの列のみ1
...
if __name__ == "__main__":
    ...
    model = ESN(train_input.shape[1], train_output.shape[1], N_x,
                density=0.05, input_scale=1.0e-1, rho=0.1, fb_scale=0.0)
    ...
    # 混同行列
    cm_train = confusion_matrix(train_label, pred_train, range(n_motion))
    ...
    # 混同行列
    cm_test = confusion_matrix(test_label, pred_test, range(n_motion))
    ...
    df_cm_train = pd.DataFrame(cm_train, range(n_motion), range(n_motion))
    ...
    df_cm_test = pd.DataFrame(cm_test, range(n_motion), range(n_motion))
