...
class QGC:
    ...
    def __call__(self, d, x):
        ...
        for i in range(self.N_x):
            ...
            self.delta_Wout[i] = (1 / absolute_value(self.Wout[i]) / absolute_value(self.Wout[i])) * ((inner_product(self.delta_theta, \
                rot(x[i], self.Wout[i]))) * self.Wout[i] - 2 * self.delta_theta * self.Wout[i] * conjugate(x[i]))
