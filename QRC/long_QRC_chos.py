...
if __name__ == '__main__':
    ...
    data = dynamics.Runge_Kutta(x0, T_train+1, dt)
    ...
    # print("train_error", (train_error/len(train_Y))**0.5)
    ...
    # print("test_error", (test_error/len(test_Y))**0.5)
    ...
    # グラフ表示用データ
    T_disp = (-15, 15)
    t_axis = np.arange(-100, -97, dt)  # 時間軸
    ...
    # plt.plot(t_axis, train_D[:,0], color='k', label='Target')
    # plt.plot(t_axis, train_Y[:,0], color='gray', linestyle='--')
    # plt.ylabel('x')
    ...
    # plt.plot(t_axis, train_D[:,1], color='k', label='Target')
    # plt.plot(t_axis, train_Y[:,1], color='gray', linestyle='--')
    # plt.ylabel('y')
    # # plt.axvline(x=0, ymin=0, ymax=1, color='k', linestyle=':')
    # # plt.legend(bbox_to_anchor=(0, 0), loc='lower left')
    ...
    # plt.plot(t_axis, train_D[:,2], color='k', label='Target')
    # plt.plot(t_axis, train_Y[:,2], color='gray', linestyle='--')
    # plt.ylabel('z')
    # plt.xlabel('t')
    # # plt.axvline(x=0, ymin=0, ymax=1, color='k', linestyle=':')
    # # plt.legend(bbox_to_anchor=(0, 0), loc='lower left')
    ...
    # ax1.text(0.7, 1.05, 'Testing', transform=ax1.transAxes)
    plt.plot(t_axis, train_D[:,0], color='k')
    plt.plot(t_axis, train_Y[:,0], color='gray', linestyle='--')
    # plt.ylabel('Output x', fontsize=20)
    # plt.axvline(x=0, ymin=0, ymax=1, color='k', linestyle=':')
    # plt.legend(bbox_to_anchor=(0, 0), loc='lower left')
    ...
    plt.plot(t_axis, train_D[:,1], color='k')
    plt.plot(t_axis, train_Y[:,1], color='gray', linestyle='--')
    ...
    plt.plot(t_axis, train_D[:,2], color='k')
    plt.plot(t_axis, train_Y[:,2], color='gray', linestyle='--')
    # plt.ylabel('Output z', fontsize=20)
    plt.xlabel('Time t', fontsize=20)
    # plt.axvline(x=0, ymin=0, ymax=1, color='k', linestyle=':')
    # plt.legend(bbox_to_anchor=(0, 0), loc='lower left')

    #plt.savefig(f'out/result_tanh_{N_x}node_sp{rho}.png')
    plt.savefig(f'out/long_qrc_node{N_x}.png')
