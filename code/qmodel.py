...
class Input:
    ...
    def __init__(self, N_u, N_x, input_scale, seed=0):
        '''
        param N_u: 入力次元(1次元)
        param N_x: リザバーのノード数(1*N_x)
        param input_scale: 入力スケーリング
        '''
        self.Win = self.make_qvector(N_x, N_u, input_scale, seed)
        print(self.Win)

    # Winベクトル作成
    def make_qvector(self, N_x, N_u, input_scale, seed=0):
        # 一様分布に従う乱数
...
class Reservoir:
    ...
    def __init__(self, N_x, density, rho, activation_func, leaking_rate,
        ...
        self.seed = seed
        self.rho = rho
        self.W = self.make_qmatrix(N_x)
        self.x = np.full(N_x,Variable(np.array([0,0,0,0])))
        self.activation_func = activation_func
        self.alpha = leaking_rate
        self.density = density

    # W行列作成
    def make_qmatrix(self, N_x):
        rec_scale = 1.0
        np.random.seed(seed=self.seed)
        W_num = np.random.uniform(-rec_scale, rec_scale, (N_x, N_x, 4))
        W = np.full((N_x, N_x), Variable(np.array([0,0,0,0])))
        for i in range(N_x):
            for j in range(N_x):
                W_num[i][j][0] = 0
                W[i][j] = Variable(W_num[i][j])
        # スペクトル半径スケーリング(パラメータ：rho)
        W *= self.rho

        return W
        
    # リカレント結合重み行列の生成(スパースな行列)
    ...
    #     W_01 = np.array(connection)

    #     W = self.make_qmatrix(N_x)
    #     W *= W_01
    ...
    def __call__(self, x_in):
        ...
        self.x = (1.0 - self.alpha) * self.x \
                 + self.alpha * all_tanh(reservoir_rot(self.W, self.x) \
                 + x_in)
        # self.x = (1.0 - self.alpha) * self.x \
        #     + self.alpha * all_isotanh(reservoir_rot(self.W, self.x) \
        #     + x_in)
        return self.x
...
class Output:
    # 出力結合重み行列の初期化
    def __init__(self, N_x, N_y, input_scale=1.0, seed=0):
        '''
        param N_x: リザバーのノード数
        param N_y: 出力次元(1)
        param seed: 乱数の種
        ...
        # 正規分布に従う乱数
        np.random.seed(seed=seed)
        Wout_num = np.random.uniform(-input_scale, input_scale, (N_x, N_y*4))
        self.Wout = np.full(N_x, Variable(np.array([0,0,0,0])))
        for i in range(N_x):
            Wout_num[i][0] = 0
            self.Wout[i] = Variable(Wout_num[i])

    # 出力結合重み行列による重みづけ
    # 変更の必要あり(xの更新に四元数回転適応する必要あり)
    def __call__(self, x):
        '''
        param x: N_x次元のベクトル
        return: N_y次元のベクトル
        '''
        # return tanh(output_rot(x, self.Wout))
        return output_rot(x, self.Wout)
...
# 最小平均二乗（LMS）法
class LMS:
    def __init__(self, N_x, N_y, eta, update=1):
        '''
        param N_x: リザバーのノード数
        param N_y: 出力次元
    ...
    def __call__(self, d, x):
        ...
        for i in np.arange(self.update):
            e = d - np.dot(self.Wout, x)
            self.Wout += self.eta * np.dot(e, x.T)
...
class QGC:
    def __init__(self, N_x, eta):
        ...
        self.N_x = N_x
        self.eta = eta
        self.Wout = np.full(N_x, Variable(np.array([0,1,1,1])))
        self.delta_theta = np.full(N_x, Variable(np.array([0,0,0,0])))
        self.delta_Wout = np.full(N_x, Variable(np.array([0,0,0,0])))
    ...
    def __call__(self, d, x):
        ...
        for i in range(self.N_x):
            # self.delta_theta = hadamard_product(tanh(output_rot(x, self.Wout)) - d, diff_tanh((output_rot(x, self.Wout))))
            self.delta_theta = hadamard_product(output_rot(x, self.Wout) - d, Variable(np.array([0,1,1,1])))
            self.delta_Wout[i] = (1 / absolute_value(self.Wout[i]) / absolute_value(self.Wout[i])) * ((inner_product(self.delta_theta, rot(x[i], self.Wout[i]))) * self.Wout[i] - 2 * self.delta_theta * self.Wout[i] * conjugate(x[i]))
        self.Wout += self.eta * self.delta_Wout
        return self.Wout
...
class ESN:
    ...
    def __init__(self, N_u, N_y, N_x, density=0.05, input_scale=1.0,
                 rho=0.01, activation_func=np.tanh, fb_scale = None,
        ...
        param classification: 分類問題の場合はTrue（default: True）
        param average_window: 分類問題で出力平均する窓幅（default: 1）
        '''
        ...
        self.QWout = np.full((N_x, N_x), Variable(np.array([0,0,0,0])))
    ...
    def qadapt(self, U, D, optimizer):
        ...
        for i in range(6):
            for n in range(data_len):
                if n % 10 == 0:
                    print((data_len*i+n)/(data_len*6), " is done")
                x_in = self.Input(U[n][i])
                # print(x_in)
                x = self.Reservoir(x_in)
                d = D[n][i]
                d = self.inv_output_func(d)
    ...
    def qfree_run(self, U):
        ...
        for i in range(6):
            for n in range(test_len):
                ...
                # 学習後のモデル出力
                self.Output.setweight(self.QWout)
                y = self.Output(x)
                Y_pred.append(y)
