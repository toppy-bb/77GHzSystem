...
class Input:
    ...
    def __init__(self, N_u, N_x, input_scale, seed=0):
        ...
        self.Win = np.random.uniform(-input_scale, input_scale, (N_x, N_u, 4))
...
class Reservoir:
    ...
    def __init__(self, N_x, density, rho, activation_func, leaking_rate,
        ...
        self.x = np.zeros((N_x,4))  # リザバー状態ベクトルの初期化
    ...
    def __call__(self, x_in):
        ...
        self.x = (1.0 - self.alpha) * self.x \
                 + self.alpha * self.activation_func(rot(self.W, self.x) \
                 + x_in)
        # print(self.x)
        return self.x
...
class Output:
    ...
    def __init__(self, N_x, N_y, seed=0):
        ...
        # 正規分布に従う乱数
        np.random.seed(seed=seed)
        self.Wout = np.random.normal(size=(N_y, N_x,4))
...
class SGD:
    def __init__(self, N_x, N_y, lr , seed = 0 , input_scale = 1e-1):
        ...
        self.N_y = N_y
        np.random.seed(seed=seed)
        self.Wout = np.random.uniform(-input_scale, input_scale, (N_y, N_x, 4))
        self.delta_theta = np.random.uniform(-input_scale, input_scale,[N_y, 4])
        self.delta_wout = np.random.uniform(-input_scale, input_scale, (N_y, N_x, 4))
        
    # Woutの更新
    def __call__(self, d, x):
        for i in range(self.N_x):
            # print(2*outer_product(outer_product(self.delta_theta[0],self.Wout[0,i]),conjugation(x[i])))
            self.delta_theta = (rot(self.Wout,x)-d)*rot(self.Wout,x) #no problem
            self.delta_wout[0,i] = (((inner_product(self.delta_theta,outer_product(outer_product(self.Wout[0,i],x[i]),conjugation(self.Wout[0,i]))))/np.linalg.norm(self.Wout[0,i])**2)*self.Wout[0,i] \
                - outer_product(outer_product(self.delta_theta[0]/(1/2),self.Wout[0,i]),conjugation(x[i])))/(np.linalg.norm(self.Wout[0,i]))
        self.Wout += self.lr * self.delta_wout
        return self.Wout
...
class ESN:
    # 各層の初期化
    def __init__(self, N_u, N_y, N_x, density=1, input_scale=1e-2,
                 rho=0.5, activation_func=np.tanh, fb_scale = None,
                 fb_seed=0, noise_level = None, leaking_rate=0.8,
                 output_func=identity, inv_output_func=identity,
                 classification = False, average_8indow = None):
        '''
        param N_u: 入力次元
        param N_y: 出力次元
        param N_x: リザバーのノード数
...
#                 H_new[col,row] += outer_product(Q[col,nth],H[row,nth])
